/**
 * VOLITIX AG - CORE APPLICATION ROUTER (UNIFIED)
 * This single file contains all exhaustive compliance forms, UI components, 
 * and routing logic. By keeping everything in one file, we bypass all import 
 * and resolution errors in both preview environments and Vercel deployments.
 */
...
import { getFirestore, collection, addDoc, onSnapshot, deleteDoc, doc, setDoc, getDocs, getDoc, updateDoc } from 'firebase/firestore';
...
const appId = typeof __app_id !== 'undefined' ? String(__app_id).split('/')[0].replace(/[^a-zA-Z0-9_-]/g, '_') : 'aviation-compliance-hub';
...
const defaultLogState = { date: new Date().toLocaleDateString('en-CA'), startTime: '08:00', endTime: '10:00', selectedAircraft: '', customer: '', locationName: '', totalAcreage: '', treatedAcreage: '', whatWasTreated: '', customCrop: '', targetPest: '', nozzleDesc: '', targetDistance: '', pumpPressure: '', travelSpeed: '', speedUnit: 'mph', driftPractices: '', coordType: 'Decimal', latDec: '', lonDec: '', latDecDir: 'N', lonDecDir: 'W', latDMS: { d: '', m: '', s: '', dir: 'N' }, lonDMS: { d: '', m: '', s: '', dir: 'W' }, coordinates: '', chemical: '', appRate: '', windSpeed: '', windDirection: 'N', temp: '', tempUnit: 'F', humidity: '', incidents: '', damageDescription: '', flightTimeValue: '', flightTimeUnit: 'Hours', flightTimeMinutes: '', kmlData: null, kmlFileName: '', attachedNotam: '' };
...
const SettingsTab = ({ company, setCompany, handleCompanySettingsSave, customUser, setCustomUser, notify }) => {
...
const ScheduleTab = ({ workOrders, isEditing, state, setState, onCancel, onSubmit, customers, products, customUser, notify }) => {
    ...
    return (
      <FormCard title={state.id ? "Edit Work Order" : "Create Work Order"} icon={CalendarDays} onSubmit={(e) => onSubmit(e, null, null)} onCancel={onCancel} submitLabel={state.status === 'Completed' ? "Save & Log Mission" : "Save Schedule"}>
         <div className="grid grid-cols-1 md:grid-cols-2 gap-6 min-w-0">
           ...
           <div className="space-y-2 min-w-0 group md:col-span-2">
             <label className="text-[10px] text-slate-400 font-black uppercase tracking-widest pl-1 block group-focus-within:text-[#a3e635] transition-colors">Boundary (KML/XML Upload)</label>
               ...
               <input 
                 ...
                 onChange={e => { 
                   ...
                   if(file) { 
                     ...
                     reader.onload = (evt) => setState({...state, kmlData: evt.target.result, kmlFileName: file.name}); 
                     reader.readAsText(file); 
                   } 
                 }} 
                 className="w-full bg-slate-950/50 border border-slate-800 p-3.5 rounded-2xl text-sm text-slate-400 outline-none file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-[10px] file:font-black file:uppercase file:tracking-widest file:bg-[#a3e635]/10 file:text-[#a3e635] hover:file:bg-[#a3e635]/20 transition-all cursor-pointer shadow-inner" 
               />
               {state.kmlFileName && <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[9px] text-[#a3e635] font-bold uppercase truncate max-w-[100px]">{state.kmlFileName}</span>}
             </div>
           </div>
         </div>
      </FormCard>
...
const CustomersTab = ({ items, logs, isEditing, state, setState, onCancel, onSubmit, onDelete }) => {
...
const MissionLogTab = ({ logs, fleet, customers, products, crops, isEditing, state, setState, onCancel, onSubmit, onDelete }) => {
  if (isEditing) {
     return (
      <FormCard title={state.id ? 'Edit Mission Record' : 'Log New Mission'} icon={Plane} onSubmit={onSubmit} onCancel={onCancel} submitLabel="Save To Vault">
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 border-b border-slate-800 pb-6">
             <Select label="Aircraft (N-Number)" value={state.selectedAircraft} onChange={e => setState({...state, selectedAircraft: e.target.value})}>
                <option value="">Select Aircraft...</option>
                {(fleet || []).map(f => <option key={f.id} value={f.id}>{f.id} - {f.model}</option>)}
             </Select>
             ...
             <Input label="Field Name / Block" value={state.locationName} onChange={e => setState({...state, locationName: e.target.value})}/>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 border-b border-slate-800 pb-6">
             <Input label="Crop/Target Area" value={state.whatWasTreated} onChange={e => setState({...state, whatWasTreated: e.target.value})} />
             <Input label="Target Pest / Disease" value={state.targetPest} onChange={e => setState({...state, targetPest: e.target.value})} />
             <Input label="Target Distance (Height)" value={state.targetDistance} onChange={e => setState({...state, targetDistance: e.target.value})} rightElement={<span className="text-[9px] font-bold text-slate-500">FT</span>}/>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 border-b border-slate-800 pb-6">
             ...
             <Input label="Pump Pressure" value={state.pumpPressure} onChange={e => setState({...state, pumpPressure: e.target.value})} rightElement={<span className="text-[9px] font-bold text-slate-500">PSI</span>}/>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-b border-slate-800 pb-6">
             <Input label="Drift Mitigation Practices" placeholder="e.g. Lowered altitude, used coarse nozzles" value={state.driftPractices} onChange={e => setState({...state, driftPractices: e.target.value})} />
          </div>

          <div className="md:col-span-3">
              <TextArea label="Incidents / Damage / Observations" value={state.incidents} onChange={e => setState({...state, incidents: e.target.value})} />
          </div>
        </div>
      </FormCard>
    );
...
const NotamTab = ({ notams, isEditing, state, setState, onCancel, onSubmit, onDelete }) => {
  ...
  if (isEditing) return (
      <FormCard title="Log Filed NOTAM" onSubmit={onSubmit} onCancel={onCancel} submitLabel="Archive NOTAM">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 border-b border-slate-800 pb-6 mb-6">
           <Input label="Latitude (Decimal)" type="number" step="any" placeholder="38.3364" value={state.latDec} onChange={e => setState({...state, latDec: e.target.value})} />
           <Input label="Longitude (Decimal)" type="number" step="any" placeholder="-90.1498" value={state.lonDec} onChange={e => setState({...state, lonDec: e.target.value})} />
           <Input label="Radius (NM)" type="number" step="any" placeholder="0.5" value={state.radius} onChange={e => setState({...state, radius: e.target.value})} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Input type="time" label="Local Start Time" value={state.localStartTime} onChange={e => setState({...state, localStartTime: e.target.value})} required/>
          <Input label="Duration (Hours)" type="number" value={state.duration} onChange={e => setState({...state, duration: e.target.value})} required/>
...
const FaaReportTab = ({ company, fleet, certs, monthlyReport, reportMonth, setReportMonth, notify, faaReports, onDelete }) => {
  return (
    <div className="space-y-8 animate-fade-in min-w-0">
...
export default function App() {
  ...
  const [authForm, setAuthForm] = useState({ companyName: '', username: '', email: '', password: '', role: 'Manager' });
  const [showPassword, setShowPassword] = useState(false);

  const [activeTab, setActiveTab] = useState('dashboard');
  const [loading, setLoading] = useState(true);
  const [notification, setNotification] = useState({ show: false, message: '', type: 'info' });
  const [deleteConfirmModal, setDeleteConfirmModal] = useState({ show: false, col: null, id: null });
  ...
  useEffect(() => {
    const initAuth = async () => {
      try { await signInAnonymously(auth); } catch (err) { console.error("Auth Exception:", err); }
    };
    initAuth();
  ...
  useEffect(() => {
    if (!user) return;
    const sub = (colName, setFn) => onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', colName), s => setFn(s.docs.map(d => ({id: d.id, ...d.data()}))), e => console.warn(`DB Sync Ignored for ${colName}:`, e.message));
    const unsubscribers = [
  ...
  useEffect(() => {
     ...
     return onSnapshot(doc(db, 'artifacts', appId, 'public', 'data', 'saas_companies', compDocId), (snap) => { if (snap.exists()) setCompany(snap.data()); }, (e) => console.warn("Company sync error:", e.message));
  }, [customUser]);
  ...
  const deleteItem = (col, id) => setDeleteConfirmModal({ show: true, col, id });
  const executeDelete = async () => {
    if (!deleteConfirmModal.col || !deleteConfirmModal.id) return;
    try { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', deleteConfirmModal.col, deleteConfirmModal.id)); notify("Record Deleted", "info"); } 
    catch (err) { notify("Error deleting record.", "error"); }
    setDeleteConfirmModal({ show: false, col: null, id: null });
  };

  const handleLogin = async (e) => {
    e.preventDefault();
    let currentUser = user;
    if (!currentUser) { try { const cred = await signInAnonymously(auth); currentUser = cred.user; setUser(cred.user); } catch(err) { return notify(`Connection Blocked: ${err.message}`, "error"); } }
    const cName = authForm.companyName?.trim().toLowerCase(); const uName = authForm.username?.trim().toLowerCase();
    try {
  ...
  const handleRegister = async (e) => {
    ...
    if (!currentUser) { try { const cred = await signInAnonymously(auth); currentUser = cred.user; setUser(cred.user); } catch(err) { return notify(`Connection Blocked: ${err.message}`, "error"); } }
    try {
       const cName = authForm.companyName?.trim(); const uName = authForm.username?.trim();
       const snap = await getDocs(collection(db, 'artifacts', appId, 'public', 'data', 'saas_users'));
  ...
  if (loading) return <div className="flex h-screen items-center justify-center bg-[#020617] text-[#a3e635] font-black uppercase tracking-widest"><GlobalStylesComp/>Initializing Terminal...</div>;
      ...
      <div className="flex min-h-screen items-center justify-center p-6 bg-[#020617] relative overflow-y-auto">
        ...
        <div className="w-full max-w-lg animate-fade-in relative z-10 my-8 border-x border-[#a3e635]/10 bg-[#020617]/40">
           <div className="text-center flex flex-col items-center justify-center px-8 pt-12 pb-8 w-full">
              <div className="w-[64px] h-[64px] rounded-full flex items-center justify-center mb-6 border border-[#a3e635]/50 bg-transparent" style={{ boxShadow: '0 0 15px rgba(163,230,53,0.1), inset 0 0 15px rgba(163,230,53,0.05)' }}><Lock className="text-[#a3e635]" size={28} strokeWidth={1.5} /></div>
              <h1 className="text-5xl md:text-[3.5rem] font-black uppercase whitespace-nowrap text-[#cbf464] mb-4 flex justify-center w-full" style={{ fontFamily: '"Orbitron", sans-serif', textShadow: '0 0 15px rgba(163,230,53,0.6), 0 0 30px rgba(163,230,53,0.3)' }}>Spray Ops</h1>
              <p className="text-slate-400 text-[10px] font-bold uppercase tracking-[0.4em] mb-8 text-center w-full">Flight Command Center</p>
              <div className="h-[1px] w-12 bg-gradient-to-r from-transparent via-[#a3e635]/60 to-transparent mb-8"></div>
              <p className="text-[#a3e635] text-[11px] font-bold uppercase tracking-widest mb-4 drop-shadow-[0_0_5px_rgba(163,230,53,0.5)]">{authMode === 'login' ? 'Secure Login' : authMode === 'register' ? 'Register Access' : 'Account Recovery'}</p>
           </div>
           <div className="px-8 pb-12">
             ...
             <div className="flex flex-col gap-4 text-center pt-8 border-t border-slate-800/50 mt-8">
                <button type="button" onClick={() => setAuthMode(authMode === 'login' ? 'register' : 'login')} className="text-[10px] text-slate-400 hover:text-slate-200 uppercase font-black tracking-widest transition-colors">{authMode === 'login' ? 'Need Account? Register New User' : 'Already have access? Login Here'}</button>
             </div>
           </div>
        </div>
      </div>
  ...
  return (
    <div className="flex h-screen bg-[#020617] overflow-hidden font-sans selection:bg-[#a3e635] selection:text-[#020617] relative">
      <GlobalStylesComp /><Toast notification={notification} />
      ...
      {deleteConfirmModal.show && (
         <div className="fixed inset-0 z-[200] bg-black/80 flex items-center justify-center p-4 animate-fade-in">
           <Card className="max-w-sm w-full border-red-500/30">
             <div className="flex items-center gap-3 text-red-500 mb-4"><AlertTriangle size={24} /><h3 className="text-xl font-black uppercase tracking-widest">Confirm Deletion</h3></div>
             <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-8 leading-relaxed">This record will be permanently removed from the database.</p>
             <div className="flex gap-4">
                <Button variant="secondary" className="flex-1 py-4" onClick={() => setDeleteConfirmModal({ show: false, col: null, id: null })}>Cancel</Button>
                <Button variant="danger" className="flex-1 py-4" onClick={executeDelete}>Delete</Button>
             </div>
           </Card>
         </div>
      )}
      ...
      <div className="w-20 lg:w-64 bg-slate-900/80 backdrop-blur-2xl border-r border-slate-800 flex flex-col shrink-0 z-30 relative">
        <div className="p-4 lg:p-6 border-b border-slate-800/50 min-w-0 flex flex-col items-center text-center relative z-10">
          <div className="hidden lg:flex flex-col items-center w-full"><h1 className="font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400 uppercase tracking-tighter w-full truncate">{company.name || 'Organization'}</h1><span className="text-[10px] text-[#a3e635] font-black uppercase tracking-[0.3em] mt-1.5" style={{ fontFamily: '"Orbitron", sans-serif' }}>Spray Ops</span></div>
          <ShieldCheck className="lg:hidden mx-auto text-[#a3e635] shrink-0" size={24} strokeWidth={2.5}/>
        </div>
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto relative z-10">
          {tabsConfig.map(t => {
            const Icon = t.icon;
            return (
              <button key={t.id} onClick={() => setActiveTab(t.id)} className={`w-full flex items-center justify-center lg:justify-start p-3.5 rounded-2xl transition-all duration-300 min-w-0 group ${activeTab === t.id ? 'bg-gradient-to-r from-[#a3e635] to-[#84cc16] text-[#020617] shadow-[0_0_20px_rgba(163,230,53,0.3)] font-black' : 'hover:bg-slate-800/50 text-slate-400 hover:text-slate-200'}`}>
                 <Icon size={18} className={`shrink-0 transition-transform duration-300 ${activeTab === t.id ? 'scale-110' : 'group-hover:scale-110'}`}/><span className={`hidden lg:inline text-[11px] uppercase tracking-widest ml-4 truncate ${activeTab === t.id ? 'font-black' : 'font-bold'}`}>{t.label}</span>
              </button>
            );
          })}
        </nav>
        <div className="p-4 border-t border-slate-800/50 space-y-2 min-w-0 relative z-10">
           <button onClick={() => {setCustomUser(null); setAuthForm({...authForm, password: ''});}} className="w-full p-3 flex items-center justify-center lg:justify-start rounded-xl text-red-500/80 hover:text-red-400 hover:bg-red-500/10 transition-all min-w-0 group"><LogOut size={16} className="shrink-0 group-hover:scale-110 transition-transform"/><span className="hidden lg:inline text-[10px] font-black uppercase tracking-widest ml-3 truncate">Sign Out</span></button>
        </div>
      </div>
      ...
      <main className="flex-1 overflow-y-auto p-6 lg:p-10 relative z-20 min-w-0">
        ...
        {activeTab === 'faa-report' && <FaaReportTab company={company} fleet={company.fleet || []} certs={certs} monthlyReport={faaReportData} reportMonth={reportMonth} setReportMonth={setReportMonth} notify={notify} faaReports={faaReports} onDelete={deleteItem} />}
      </main>
    </div>
  );
}
